...
#Bandwidth
LORA_BW_0200                            = const(0x34)
LORA_BW_0400                            = const(0x26)
LORA_BW_0800                            = const(0x18)
LORA_BW_1600                            = const(0x0A)
...
LORA_CR_LI_4_5                          = const(0x05)
LORA_CR_LI_4_6                          = const(0x06)
LORA_CR_LI_4_7                          = const(0x07)
...
class SX1280:
    ...
    def __init__(self, spi, cs, reset, busy, frequency, *, preamble_length=8, baudrate=1500000, debug=False, txen=False, rxen=False):
        ...
        print('doing first default config')
        self.default_config()
    ...
    def spi_readinto(self, buf, end=0):
        self._cs.value(0)
        self._device.readinto(buf, end)
        self._cs.value(1)
        
    def spi_write_readinto(self, wbuf, rbuf, out_end=0, in_end=0):
        self._cs.value(0)
        rb = bytearray(out_end)
        self._device.write_readinto(wbuf, rb)
        self._cs.value(1)
        for i in range(out_end):
            rbuf[i] = rb[i]
    ''' ----------------------------------- '''
    ...
    def _send_command(self,command,stat=False):
        ...
        #self.spi_write_readinto(command, self._BUFFER, out_end=_size, in_end=_size)

        '''
            写入时最后面多写一个byte, 读取就自然跳过返回的status byte
        '''
        self._cs.value(0)
        buffer = bytearray(_size)
        self._device.write_readinto(command, buffer)
        self._cs.value(1)
   
        if stat:
            print('SendCMD CMD:',[hex(i) for i in command])
            print('SendCMD BUF:',[hex(i) for i in buffer])
        if self._debug:
            print('\t\t{}'.format(self._convert_status(buffer[0])))
        self._busywait()
            
        #return result[:_size]	#原来这里是复制一份的意思
        return buffer
    ...
    def _writeRegister(self,address1,address2,data):
        ...
        #self._cs.value(0)
        #self._device.write(bytes([_RADIO_WRITE_REGISTER,address1,address2,data]))
        #self._cs.value(1)
    ...
    def _readRegister(self,address1,address2):
        ...
        '''
        self._cs.value(0)
        self._device.write(bytes([_RADIO_READ_REGISTER,address1,address2, 0]))
        buf = self._device.read(1)
        self._cs.value(1)
   
        if self._debug:
            [print(hex(i),' ',end='')for i in buf]
            print('')
            
        self._busywait()

        return buf[0] # TODO this seems wrong
    ...
    def _readRegisters(self,address1,address2,_length=1):
        ...
        self._cs.value(0)
        self._device.write(bytes([_RADIO_READ_REGISTER,address1,address2,0]))
        buf = self._device.read(_length)
        self._cs.value(1)
   
        #self.spi_write_readinto(bytes([_RADIO_READ_REGISTER,address1,address2,0]+[0]*_length),self._BUFFER,out_end=_size,in_end=_size)

        self._busywait()
        if self._debug: print('Read Regs ({}, {}) _BUFFER: {}'.format(hex(address1),hex(address2),[hex(i) for i in buf]))
        return buf
    ...
    def printRegisters(self,start=0x0900,stop=0x09FF,p=True):
        ...
        #self.spi_write_readinto(bytes([_RADIO_READ_REGISTER])+int(start).to_bytes(2,'big')+b'\x00'+b'\x00'*_length,buff,out_end=_size,in_end=_size)
        
        self._cs.value(0)
        '''
            写入时最后面多写一个byte, 读取就自然跳过返回的status byte
        '''
        self._device.write(bytes([_RADIO_READ_REGISTER])+int(start).to_bytes(2,'big') + b'\x00')
        buff = self._device.read(_length)
        self._cs.value(1)
    ...
    def sleep(self):
        ...
        #self.spi_write_readinto(bytes([_RADIO_SET_SLEEP, 0x07]),self._BUFFER,out_end=2,in_end=2)

        self._cs.value(0)
        self._device.write(bytes([_RADIO_SET_SLEEP, 0x07]))
        buf = self._device.read(1)
        self._cs.value(1)
        ...
        if self._debug:
            ...
            print('\t\t{}'.format(self._convert_status(buf[0])))
    ...
    def wake_up(self):
        if self.sleeping:
            ...
            sleep(0.1)
            self._cs.on()
            self._device.write(bytes([0xC0,0])) # send no-op to wake-up
            self._cs.off()
    ...
    def set_Modulation_Params(self, modParam1=LORA_SF7, modParam2=LORA_BW_0200, modParam3=LORA_CR_4_6):
    ...
    def write_Buffer(self,data):
        ...
        self.spi_write(bytes([_RADIO_WRITE_BUFFER,_offset])+data)

    def read_Buffer(self,offset,payloadLen):
        #_payload = bytearray(payloadLen)

        self._cs.value(0)
        self._device.write(bytes([_RADIO_READ_BUFFER, offset])) #多写一个数据，读取自然跳过status byte，读取的全部为数据了
        #self.spi_readinto(_payload, end=payloadLen)
        _payload = self._device.read(payloadLen)
        self._cs.value(1)
    ...
    def dump_buffer(self, dbuffer):
        ...
        #self.spi_write(bytes([_RADIO_READ_BUFFER,0,0]), end=3)
        #self.spi_readinto(dbuffer)

        self._cs.value(0)
        self.spi_write(bytes([_RADIO_READ_BUFFER, 0x00, 0x00])) #多写一个数据，读取自然跳过status byte，读取的全部为数据了
        #self.spi_readinto(_payload, end=payloadLen)
        self._device.readinto(dbuffer)
        self._cs.value(1)

        #print('Status:',self._convert_status(self._BIGBUFFER[0]))
        [print(hex(i),end=',') for i in dbuffer]
        print('')
    ...
    def get_Rx_Buffer_Status(self):
        buf = self._send_command(bytes([_RADIO_GET_RXBUFFERSTATUS,0x00,0x00,0x00]))
        #return buf[:4]		# 不需要复制数据了，新的send_command返回都是独立内存数据
        return buf
    ...
    def status(self, raw=False):
        ...
        #self.spi_write_readinto(bytes([_RADIO_GET_STATUS]),self._BUFFER)
        self._cs.value(0)
        result = bytearray(1)
        self._device.write_readinto(bytes([_RADIO_GET_STATUS]), result)
        self._cs.value(1)
        ...
        if raw:
            return result[0]
        self._busywait()
        return self._convert_status(result[0])

    def send_mod(self, data, *,
        keep_listening=False,
        header=False, destination=None, node=None, identifier=None, flags=None, debug=False ):
    ...
    def get_range(self,addr=[0,0,0,0],raw=False,timeout=10,t_resend=3,debug=False,delay=1):
        ...
        if timeout is not None:
            resend = time()+t_resend
            timed_out = time()+timeout
            while time() < timed_out:
        ...
        if timeout is not None:
            ...
            while time() < timed_out:
                ...
                if time() > resend:
                    ...
                    resend=time()+t_resend
            print('SLAVE timed out {}'.format(time()))
    ...
    def send_fast(self,data,l):
        ...
        self._cs.value(0)
        self._device.write(b'\x1a\x00'+data)
        self._cs.value(1)
